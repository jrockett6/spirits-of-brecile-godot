...
uniform float height_min = 0.04;
uniform float height_max = 0.08;

uniform float width_min = 0.01;
uniform float width_max = 0.02;
...
void vertex() {
	...
	float y_shifted = clamp(VERTEX.y - 0.5, 0.0, 1.0);
	
	// Create wind from worley noise and world UVs
	vec2 uv = (WORLD_MATRIX * vec4(VERTEX,-1.0)).xz * wind_scale;
	uv += wind_direction_normalized.xz * time;
	wind = pow(worley2(uv),2.0) * VERTEX.y;
	...
	float sway_pitch = ((deg_sway_pitch * DEG2RAD) * wind) * .5;
	float sway_yaw = ((deg_sway_yaw * DEG2RAD) * sin(time) * wind) * .5;
...
//	COLOR = vec4(WORLD_MATRIX * vec4(vertex, -1.0));
//	COLOR = vec4(vec3(worley2(POSITION.xy)), 1.0);
//	COLOR = vec4(worley2((WORLD_MATRIX * vec4(VERTEX.xz, 1.0, 1.0) * .1).xz));

	NORMAL = UP;
	COLOR = mix(color_bottom, color_top, y_shifted);
}
...
void fragment() {
	...
	ALBEDO = COLOR.rgb;
	SPECULAR = 0.5;
	ROUGHNESS = clamp(1.0 - (wind * 1.2), roughness_min, 1.0);
}